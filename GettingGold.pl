...
addToQueue(X,Y,W,H):-X>=0,Y>=0,X<W,Y<H,not(visited(X,Y)),not(queue(X,Y)),!,assert(queue(X,Y)).
...
visit(X,Y,W,H):-(visited(X,Y),!,getNextFromQueue(W,H);(field(X,Y,84),!,assert(visited(X,Y)),getNextFromQueue(W,H);field(X,Y,35),!,assert(visited(X,Y)),getNextFromQueue(W,H);field(X,Y,71),!,assert(visited(X,Y)),foundGold(Old),retract(foundGold(Old)),NewGold is Old+1,assert(foundGold(NewGold)),addNextToQueue(X,Y,W,H),getNextFromQueue(W,H);assert(visited(X,Y)),addNextToQueue(X,Y,W,H),getNextFromQueue(W,H))).

main:-set_stream(user_input,buffer_size(16384)),fill_buffer(user_input),readUInt(W),readUInt(H),assert(field(-1,-1,35)),assert(queue(-1,-1)),assert(visited(-1,-1)),assert(foundGold(0)),readField(0,H,W),field(X,Y,80),visit(X,Y,W,H),foundGold(Gold),writeln(Gold).